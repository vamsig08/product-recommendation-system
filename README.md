...
## Approach:
  ...
  -- For RAG, the dataset came with ingredient properties which I used to tag the products first ingredient to match its effects. For example "Contains Coffee Beans, which has Rich flavor; provides a natural energy boost."
  -- How it works:
...
## Areas for improvement:
  -- A lot of work can be done to make this prototype a sellable product:
    -- Utilizing real time datasets, with more products for more options and diversity.
    -- Implementing collaborative filtering which is a more common recommendation algorithm, very popular.
    -- Using LLMs for RAG implementation as I mentioned before, it provides a better grammar making descriptions sound more polished and contextualized.
    -- For this protoype, I built a straightforward user interface using React. The current UI meets the users needs, but its not visually appealing. 
